...
public class BlockSnowGrass extends BlockSnow implements ILayeredFallingBlock
{
    ...
    public static void leftClickBlock(@Nonnull PlayerInteractEvent.LeftClickBlock event) {
        ...
        if(state.getBlock() instanceof BlockSnowGrass) {
            if(!isLookingAtSnow(event.getEntityPlayer(), event.getPos(), state)) {
                if(event.getWorld().isRemote) {
                    SoundUtils.playSound(event.getWorld(), event.getPos(), SoundEvents.BLOCK_GRASS_BREAK, SoundCategory.BLOCKS, 1, 0.8f);
                    Minecraft.getMinecraft().effectRenderer.addBlockDestroyEffects(event.getPos(), state);
                }

                else {
                    if(!event.getEntityPlayer().isCreative())
                        Blocks.TALLGRASS.dropBlockAsItem(event.getWorld(), event.getPos(), Blocks.TALLGRASS.getStateFromMeta(1),
                                EnchantmentHelper.getEnchantmentLevel(Enchantments.FORTUNE, event.getEntityPlayer().getHeldItemMainhand()));
    ...
    @SubscribeEvent
    public static void createSnowGrass(@Nonnull PlayerInteractEvent.RightClickBlock event) {
        final ItemStack held = event.getItemStack();
        if(held.getItem() == Item.getItemFromBlock(Blocks.SNOW_LAYER)) {

        }

        else if(held.getMetadata() == 1 && held.getItem() == Item.getItemFromBlock(Blocks.TALLGRASS)) {
